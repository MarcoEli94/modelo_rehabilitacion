...
EXERCISE_ID = 7
EXERCISE_NAME = "Standing Shoulder Internal/External Rotation"
WINDOW_SIZE = 48
RANDOM_STATE = 42
TARGET_RECALL = 0.90
...
ARTIFACTS_DIR = PROJECT_DIR / "standing_shoulder_internal_external_rotation/standing_shoulder_internal_external_rotation_artifacts"
...
ERROR_LABELS = {
    "low_range": "codo_separado_del_torso",
    "trunk_lean": "inclinacion_excesiva_tronco",
    "shrug": "elevacion_escapular_excesiva",
    "wrist_drop": "descenso_de_muneca_en_rotacion",
...
def _normalize_frame(frame: Any, default_index: int = 0) -> dict[str, Any]:
    """Normaliza un frame para soportar tanto puntos_clave como puntos, preservando timestamp 't'."""
    if not isinstance(frame, dict):
        raise TypeError(f"Frame debe ser dict, got {type(frame)}")
    
    result = {"frame_index": default_index}
    
    # Preservar timestamp si existe
    if "t" in frame:
        result["t"] = frame["t"]
    
    # Intentar puntos_clave primero, luego puntos
    if "puntos_clave" in frame:
        result["puntos_clave"] = frame["puntos_clave"]
    elif "puntos" in frame:
        result["puntos_clave"] = frame["puntos"]
    else:
        raise KeyError(f"Frame debe contener 'puntos_clave' o 'puntos', got {list(frame.keys())}")
    
    return result


def coerce_frames(payload: Any) -> list[dict[str, Any]]:
    """Convierte múltiples formatos JSON a lista normalizada de frames con puntos_clave."""
    ...
    # Buscar contenedor de frames
    for key in ("frames", "ventana", "secuencia", "window"):
        if key in payload:
            frames = payload[key]
            return [_normalize_frame(frame, idx) for idx, frame in enumerate(frames)]
    
    # Si es un frame directo con puntos_clave o puntos
    try:
        return [_normalize_frame(payload, 0)]
    except KeyError:
        raise KeyError("No se encontró una secuencia temporal válida en el JSON.")
...
def generate_temporal_window_from_example(
    ...
    base_points = dict(frames[0]["puntos_clave"])
    ...
    if EXERCISE_ID == 7:
        target_angle = float(rng.uniform(12.0, 28.0))
        if "low_range" in active_errors:
            target_angle = float(rng.uniform(20.0, 30.0))  # Errores leves
    else:
...
def load_ui_prmd_dataset(
    ...
    if not records:
        raise FileNotFoundError(
            f"No se encontraron secuencias m{movement_id:02d} en las carpetas UI-PRMD esperadas."
        )

    if strict_all and len(records) != candidate_files:
        raise RuntimeError(
            f"No se pudieron usar todas las secuencias m{movement_id:02d} detectadas. "
...
def get_dataset() -> tuple[np.ndarray, np.ndarray, pd.DataFrame, str]:
    try:
        ...
        raise FileNotFoundError(
            f"No se encontraron secuencias m{EXERCISE_ID:02d} en las carpetas UI-PRMD del dataset/ esperadas."
        ) from exc
...
def build_tcn_model(input_shape: tuple[int, int]) -> Any:
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs, name="standing_shoulder_internal_external_rotation_tcn")
    model.compile(
...
def detect_errors(summary: dict[str, float]) -> list[str]:
    errors: list[str] = []
    if summary["max_shoulder_angle_deg"] > 45.0:
        errors.append(ERROR_LABELS["low_range"])
    if summary["trunk_tilt_p95_deg"] > 10.0:
        errors.append(ERROR_LABELS["trunk_lean"])
    if summary["shoulder_elevation_p95"] > 0.14:
        errors.append(ERROR_LABELS["shrug"])
    if summary["wrist_above_elbow_mean"] < -0.05:
        errors.append(ERROR_LABELS["wrist_drop"])
    if summary["symmetry_p95_deg"] > 20.0:
        errors.append(ERROR_LABELS["asymmetry"])
    if summary["velocity_std_deg"] > 4.2:
...
def export_artifacts(
    ...
    model_path = output_dir / "standing_shoulder_internal_external_rotation_tcn.keras"
    bundle_path = output_dir / "standing_shoulder_internal_external_rotation_bundle.joblib"
...
def train_and_export() -> dict[str, Any]:
    ...
    print("=== Standing Shoulder Internal/External Rotation / TCN ===")
